(function () {
  ...
  angular
    .module('power-plant')
    .controller('coldChain_detail_Controller', coldChain_detail_Controller);

  /** @ngInject */
  function coldChain_detail_Controller($log, $timeout, energyService, c3, $scope, computedService, $http, utilService, $rootScope, $interval, moment, $window) {
    var vm = this;
    vm._ = _;

    $log.info("# coldChain_detail_Controller");
    ...
    /*
     * @description : 우측 상단 판매처 선택했을 때 제품 리스트 호출
     * @author : HS
    ...
    function getConsumerList(drType){
      ...
      // }).then(function (resp) {
        ...
        //if(0 == drType){
        //   vm.products = resp.data.data;   //전체이면 수용가리스트 뿌리기
          ...
          // }
          var cateList = ["육류", "수산물", "청과류", "의약품", "유제품", "채소류", "계란류"];
          var cateCIxList = [78, 35, 84, 54, 42, 61, 80];

          for(var i=0 ; i < cateList.length ; i++){
            vm.categories.push({
              cate_idx: i,
              cate_name: cateList[i],
              cold_index: cateCIxList[i]
            });
          }

          var proList = ["소고기", "돼지고기", "닭고기", "오리고기", "가공육", "수입육"];
          var cIxList = [70, 60, 30, 90, 50, 80];
          var underList = [2, 3, 6, 0, 4, 1];

          for(var i=0 ; i < proList.length ; i++){
            vm.products.push({
              pro_name: proList[i],
              cold_index: cIxList[i],
              st_temp: -2,
              trans_num: 10,
              chain_data: 9,
              under_st: underList[i]
            });
          }

          $log.debug("vm.maxAvailNegaWatt : " + vm.maxAvailNegaWatt);
        // }
        // else setProDetailList(vm.resourcesConsumers[0].cons_idx);    //수용가 동 리스트 호출
    ...
    vm.clickedR = function () {
      ...
      //if(vm.drType != 0) setProDetailList(vm.resourcesConsumers[vm.consumerBeginNumber].cons_idx);
    };
    ...
    /*
     * @description : 제품 리스트 셋팅
     * @author : HS
     * @param consumer Idx
     */
    function setProDetailList(consIdx){
      vm.consIdx = consIdx;
      ...
      $log.info("#setProDetailList start. consIdx : " + consIdx);
      ...
      // }).then(function (resp) {
        ...
        //카테고리에 제품 정보가 없을 경우 카테고리 정보 자체를 제품 정보로 사용
        // if(vm.products.length == 0){
        //   for(var i=0 ; i < vm.resourcesConsumers.length ; i++){
        //     if(consIdx == vm.resourcesConsumers[i].cons_idx) vm.products = [vm.resourcesConsumers[i]];
        //   }
        // }
    ...
    function drawLineChart(drType) {
      ...
      //   vm.cont_watt = resp.data.cont_watt;
        ...
        var today = new Date();
        // var toHour = today.getHours() * 4 + 1;
        var toHour = today.getHours() * 2 + 1;
        var lastDayCold = ['<어제 물동량 시간대별 콜드지수>'];
        var todayCold = ['<광교점 물동량 시간대별 콜드지수>'];
        var i=0;

        // for (i=0; i<97; i++) {
        for (i=0; i<49; i++) {
          lastDayCold.push(utilService.getRandomIntInclusive(60, 90));
        }

        for (i=0; i<toHour; i++) {
          todayCold.push(utilService.getRandomIntInclusive(60, 90));
        }

        //중앙상단 라인차트
        utilService.drawTopLineChartColdIdex('#resource-graph', todayCold, lastDayCold, vm);

        //죄측상단 미니막대그래프 8개
        utilService.drawMiniCatColdIndex('#chartbar1', vm);
